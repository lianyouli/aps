...
LastEditors: Arthur
LastEditTime: 2023-04-15 18:24:50
...
class TestPlugins(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.manager = PluginManager(directories_list=[aps.__APS_PLUGINS_HOME__],
                                     categories_filter={
                                         IFieldPlugin.categoryName: IFieldPlugin},
                                     plugin_info_ext="plugin")
        self.manager.collectPlugins()

    def test_show_plugins_info(self):
        self.assertGreater(len(self.manager.getAllPlugins()),
                           0, 'the plugins must be existing')
        for p in self.manager.getAllPlugins():
            logger.info(
                f"{p.author}, {p.name}, {p.version}, {p.website}, {p.description}")
            # logger.debug(dir(p.plugin_object))

    def test_f1messageType(self):
        logger.info(f"category: {IFieldPlugin.categoryName}")
        f1msgtype = self.manager.getPluginByName(
            "f1messagetype", category=IFieldPlugin.categoryName)
    ...
    def test_f1messageType_by_type(self):
        ...
        f1msgtype = self.manager.getPluginByName(
            F1MessageType.fieldName, category=F1MessageType.categoryName)
        self.assertIsNotNone(f1msgtype)
        ...
        logger.debug(dir(f1))
        data = ApsData(ApsRouter(destinationID="90010000",
                       sourceID="31040000", batchNo="01"), message={}, aux={})
        f1.set(data=data,
               context=ApsContext(context={}))
    ...
    def test_f2pan(self):
        ...
        f2pan = self.manager.getPluginByName(
            F2Pan.fieldName, category=F2Pan.categoryName)
    ...
    def test_f2pan_pan_and_bin(self):
        ...
        pan = "891234567890123"
        data = ApsData(ApsRouter(destinationID="90010000",
                       sourceID="31040000", batchNo="01"), message={"f2pan": F2PanRecord(pan)}, aux={})
        f2.set(data=data,
               context=ApsContext(context={}))
